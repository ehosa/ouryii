...
class Appone extends Component
{
     ...
     * @see Cookie
     */
    public $identityCookie = ['name' => '_apponeidentity', 'httpOnly' => true];
    /**
     ...
     */
    public $idParam = '_a_id';
    /**
     * @var string the session variable name used to store the value of expiration timestamp of the authenticated state.
     * This is used when [[authTimeout]] is set.
     */
    public $authTimeoutParam = '_a_expire';
    /**
    ...
    public function init()
    {
        ...
        if ($this->identityClass === null) {
            throw new InvalidConfigException('Appone::identityClass must be set.');
        }
        if ($this->enableAutoLogin && !isset($this->identityCookie['name'])) {
            throw new InvalidConfigException('Appone::identityCookie must contain the "name" element.');
        }
    ...
    public function login(IdentityInterface $identity, $duration = 0)
    {
        if ($this->beforeLogin($identity, false, $duration)) {
            ...
            if ($this->enableSession) {
                $log = "Appone '$id' logged in from $ip with duration $duration.";
            } else {
                $log = "Appone '$id' logged in from $ip. Session not enabled.";
            }
    ...
    protected function loginByCookie()
    {
        ...
        if (isset($data['identity'], $data['duration'])) {
            ...
            if ($this->beforeLogin($identity, true, $duration)) {
                ...
                $id = $identity->getId();
                $ip = Yii::$app->getRequest()->getUserIP();
                Yii::info("Appone '$id' logged in from $ip via cookie.", __METHOD__);
    ...
    public function logout($destroySession = true)
    {
        ...
        if ($identity !== null && $this->beforeLogout($identity)) {
            ...
            $id = $identity->getId();
            $ip = Yii::$app->getRequest()->getUserIP();
            Yii::info("Appone '$id' logged out from $ip.", __METHOD__);
